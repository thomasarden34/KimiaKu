...
// 6. Fungsi Utama Mengatur Mood & Aksi Robot
...
function speakText(text) {
  if ('speechSynthesis' in window) {
    // Batalkan proses bicara sebelumnya agar tidak tumpang tindih
    ...
    // Jika tidak didukung, buat fallback manual menggunakan timer agar mulut tetap bergerak sementara
    setTimeout(stopSpeaking, 3000);
  }
}

// 8. Fungsi Memulai Animasi Bicara Robot
function startSpeaking(mood) {
  ...
  // Pilih Frasa Acak Berdasarkan Mood, Tampilkan, dan Suarakan
...
// Loop Kedipan Mata Otomatis setiap beberapa detik
setInterval(() => { if (!isSpeaking && curMood !== 'wink') doBlink(); }, 3000 + Math.random() * 2000);

// Set Kondisi Utama Wajah Pertama Kali Dimuat
applyMood('normal');
